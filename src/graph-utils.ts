import { type Node, type Edge, type ContinuumState } from './types';
 ...
 */
export const getChildren = (state: ContinuumState, nodeId: string): Node[] => {
 ...
 */
export const getParents = (state: ContinuumState, nodeId: string): Node[] => {
 ...
 */
export const getRelatedNodes = (state: ContinuumState, nodeId: string): Node[] => {
 ...
 */
export const wouldBeCircular = (state: ContinuumState, sourceId: string, targetId: string): boolean => {
  if (sourceId === targetId) return true;
  
  const parents = getParents(state, sourceId);
  for (const parent of parents) {
    if (parent.id === targetId) return true;
    if (wouldBeCircular(state, parent.id, targetId)) return true;
  }
